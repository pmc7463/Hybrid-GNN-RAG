...
class Llama(BaseLanguageModel):
    ...
    def load_model(self, **kwargs):
        model = LlamaTokenizer.from_pretrained(**kwargs, use_fast=False, token=hf_token)
        return model
    ...
    def prepare_for_inference(self, **model_kwargs):
        ...
        use_fast=False, token=hf_token)
        #model_kwargs.update({'use_auth_token': True})
        print("model: ", self.args.model_path)
        self.generator = pipeline("text-generation", token=hf_token, model=self.args.model_path, tokenizer=self.tokenizer, device_map="auto", model_kwargs=model_kwargs, torch_dtype=self.DTYPE.get(self.args.dtype, None))
